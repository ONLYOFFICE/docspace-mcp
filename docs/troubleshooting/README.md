# ONLYOFFICE Apps MCP server troubleshooting

This section helps you diagnose and resolve issues when configuring and interacting with the ONLYOFFICE Apps MCP server. It covers both common issues that apply to any MCP client and client-specific guidance for popular integrations:

- [Common issues you can encounter with any of the MCP clients](common-issues.md)
- Client-specific issues affecting
    - [ONLYOFFICE Desktop Editors](onlyoffice-troubleshooting.md),
    - [Claude](claude-troubleshooting.md),
    - [Cursor](cursor-troubleshooting.md),
    - [Le Chat](le-chat-troubleshooting.md),
    - [VS Code](vs-code-troubleshooting.md), and
    - [Windsurf](windsurf-troubleshooting.md).
